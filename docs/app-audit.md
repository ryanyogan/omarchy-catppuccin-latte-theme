...
| Herdr 0.8.2 | Follow-up in the persistent Foot session reproduced dark tab and active-row backgrounds despite automatic Latte/dark configuration; a config reload alone did not fix them. Installed the optional complete palette template and theme-set sync hook, disabling host-based auto-switching. Reload returned no diagnostics. Live before/after inspection confirms light tab/sidebar surfaces, lavender active rows and separators, readable labels and a contrasting purple active tab. Rendered both Latte and dark Catppuccin through Omarchy's actual template generator; checked all 19 supported tokens, config preservation and repeated-hook stability. |
...
Herdr follow-up references: [0.8.2 custom color schema](https://github.com/herdrdev/herdr/blob/v0.8.2/src/config/theme.rs), [effective theme resolution](https://github.com/herdrdev/herdr/blob/v0.8.2/src/app/mod.rs), [palette roles](https://github.com/herdrdev/herdr/blob/v0.8.2/src/app/state.rs), and [active-tab foreground behavior](https://github.com/herdrdev/herdr/blob/v0.8.2/src/ui/widgets.rs). For future TUI audits, inspect a persistent attached session as well as a fresh launch, and check inactive tabs, focused rows, navigation selections, popup surfaces, separators and status labels. A light terminal background alone does not establish that app-owned UI colors switched successfully.
